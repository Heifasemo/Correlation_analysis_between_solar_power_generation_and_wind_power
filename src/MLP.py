# -*- coding: utf-8 -*-
"""
Residual Dual-Branch MLP + LightGBM hybrid model
for solar power prediction with softmax centroid fusion.
...
class Config:
    ...
    # GHI bins for local softmax centroid fusion
...
# ============================================================
# 3. Dual-Branch Residual MLP definition
# ============================================================
class DualBranch_ResMLP(nn.Module):
    """
    Residual MLP with:
      - Main branch for overall features
      - Secondary branch for wind features
      - GHI-specific branch
    """
...
# ============================================================
# 4. Data loading and feature derivation
# ============================================================
def load_and_derive(path: str):
    """Load CSV and derive extended GHI features."""
    df = pd.read_csv(path)
    df["ghi_sq"] = df["ghi"] ** 2
    df["ghi_cu"] = df["ghi"] ** 3
...
# ============================================================
# 5. Learnable softmax fusion weights
# ============================================================
def fit_softmax_weights(y_val, preds_list, loss_type="huber",
                        max_steps=3000, lr=1e-2):
    """
    Learn optimal softmax weights for model fusion on validation set.
    """
    ...
    logits = torch.zeros(P.shape[1], device=device, requires_grad=True)
...
# ============================================================
# 6. Training, validation, and model saving
# ============================================================
def train_and_eval(train_csv: str, test_csv: str):
    """Train MLP + LightGBM hybrid and evaluate."""
    ...
    # 4) Train RES-MLP with early stopping (R² criterion)
    ...
    best_r2, patience_counter, best_state = -np.inf, 0, None
    for ep in range(1, Config.nn_epochs + 1):
        ...
        if r2v > best_r2 + 1e-4:
            best_r2, patience_counter, best_state = r2v, 0, model.state_dict()
        else:
            patience_counter += 1
            if patience_counter >= Config.patience:
                print(f"Early stopping at Ep{ep} (best R²={best_r2:.4f})")
                break

    # 5) Save MLP and scalers
    ...
    lgbm.fit(
        ...
        eval_metric="rmse",
        callbacks=[
            lgb.early_stopping(stopping_rounds=Config.lgbm_es),
            lgb.log_evaluation(period=50),
        ],
    )
    joblib.dump(lgbm, "model/lgbm.pkl")

    # 7) Learn global and local softmax fusion weights
    model.eval()
    with torch.no_grad():
    ...
    for b in np.unique(bin_ids):
        ...
        if idx.sum() >= Config.min_bin_count:
            w_b = fit_softmax_weights(y_va[idx],
                                      [val_pred_nn[idx], val_pred_gbm[idx]],
                                      loss_type="huber")
    ...
    # 8) Evaluate on test set and export comparison CSV
    Xte_a, Xte_s, y_te, _ = load_and_derive(test_csv)
    ...
    print("Columns: pred_mlp, pred_gbm, pred_soft (global), pred_soft_local (local)")

    # 9) Save model configuration for inference consistency
    cfg = {
    ...
    print(" Model configuration saved to model/model_config.json")
...
if __name__ == "__main__":
    # Default file paths (update as needed)
